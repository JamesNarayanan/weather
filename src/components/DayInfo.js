...
class DayInfo extends Component {
	...
	render() {
		...
		var temp =
			(day.apparentTemperatureHigh + day.apparentTemperatureLow) / 2;
			...
			<div className="row mx-2 mb-3">
				<div className="col-12 col-md-6 mb-3 mb-md-0 d-flex flex-column text-center text-md-left">
					...
					<div className="h3 text-capitalize">
						{day.precipProbability >= 0.3
							? day.precipType
							: "No precipitation"}
					</div>
					<div className="display-1 mt-n3 mb-n2">
						{Math.round(temp)}°
					</div>
					<div>{day.summary}</div>
				</div>
				<div
					className={`col-12 col-md-6 ${textSizing} font-weight-normal d-flex flex-column text-center text-md-left`}
				>
					<div>
						Precipitation: {Math.round(day.precipProbability * 100)}
						%
					</div>
					{precipAccumulation &&
					day.precipType === "snow" &&
					...
					<div>
						Wind Speed: {windSpeed.toFixed(2)} {windSpeedUnit}
					</div>
				</div>
			</div>
		);
