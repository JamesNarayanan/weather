...
class Day extends Component {
	render() {
		...
		return (
			<div
				id="dayWrapper"
				className="m-2 px-0 py-2 d-flex flex-column align-items-center rounded"
				onClick={() => this.props.onClick(this.props.dayNum)}
				style={{
					backgroundColor:
						this.props.selectedDay === this.props.dayNum
							? "rgba(0,0,0,.2)"
							: "",
					width: "100px"
				}}
			>
				<div className="h5 mb-0">
					{this.props.dayNum === 0 ? "Today" : getWeekday(day.time)}
				</div>
				<Skycons
				...
				<div className="my-0 d-flex h6">
					...
					<div className="text-muted">{Math.round(low)}°</div>
				</div>
			</div>
		);
	}
